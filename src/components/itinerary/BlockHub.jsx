...
const labelCls = "text-[9px] font-mono tracking-widest text-[var(--text-muted)] uppercase mb-0.5";
...
function CheckItem({ text, done, onToggle, onDelete }) {
  ...
  return (
    ...
    >
      ...
      {hovered && (
        <button onClick={onDelete} className="text-[9px] text-[var(--text-muted)] hover:text-red-400 transition-colors shrink-0">✕</button>
      )}
    </div>
  );
}
        ...
        <FieldRow label="Cost">
          <div className="flex items-center gap-1">
            <input
              ...
              className={inputCls}
            />
            <span className="text-[9px] font-mono text-[var(--text-muted)] shrink-0">VP$</span>
          </div>
        </FieldRow>
    ...
    <div className="p-2 flex flex-col gap-2">
      {links.map((lk, i) => (
        <div key={i} className="flex items-center gap-1.5 group">
          <span className="text-[11px] font-mono text-[var(--text-secondary)] flex-1 truncate">{lk.label}</span>
          ...
          <button onClick={() => deleteLink(i)} className="text-[9px] text-[var(--text-muted)] hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all">✕</button>
        </div>
      ))}
      <div className="flex gap-1 mt-1">
    ...
    <div className="p-2 flex flex-col gap-2">
      {contacts.map((c, i) => (
        <div key={i} className="flex items-center gap-2 group">
          ...
          <span className="text-[11px] font-mono text-[var(--text-primary)] flex-1">{c.name}</span>
          {c.phone && <span className="text-[10px] font-mono text-[var(--text-muted)]">{c.phone}</span>}
          <button onClick={() => deleteContact(i)} className="text-[9px] text-[var(--text-muted)] hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all">✕</button>
        </div>
      ))}
